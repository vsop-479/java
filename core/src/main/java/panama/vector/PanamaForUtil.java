...
final class PanamaForUtil90 {
	...
	public void encode(int[] input, int bitsPerValue) throws IOException {
		switch (bitsPerValue) {
			case 1:
				fastPack1(input, tmp);
				break;
...
//			out.writeInt(tmp[i]);
	...
	public void decode(int bitsPerValue, int[] output) throws IOException {
	...
	private static void fastPack23(int[] input, int[] output) {
		...
		// 读取新的vector。
		inVec = IntVector.fromArray(SPECIES_128, input, inOff += 4);
		// inVec左移14bit，存放outVec。高18bit则存放了inVec的低18bit。
		outVec = inVec.lanewise(VectorOperators.LSHL, 14).or(outVec);
		outVec.intoArray(output, outOff);
		outOff += 4;
		// inVec右移掉已经存放的18bit，存放5bit到outVec。
		outVec = inVec.lanewise(VectorOperators.LSHR, 18);
		inVec = IntVector.fromArray(SPECIES_128, input, inOff += 4);
		// inVec左移腾出5bit，存放outVec。
		outVec = inVec.lanewise(VectorOperators.LSHL, 5).or(outVec);
		inVec = IntVector.fromArray(SPECIES_128, input, inOff += 4);
